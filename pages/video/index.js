...
Page({
  ...
  data: {
    ...
    navId: '', //导航标识
    videoList: [], //视频列表
    currentVid: [], //当前点击的videoid
    videoUpdateTime: [], // 记录video播放的时长
    isTriggered:false,//下拉刷新开启标识
  },
  ...
  handlePlay(event) {
    ...
    if (videoItem) { //判断播放列表中是否已有当前video
      this.videoContext.seek(videoItem.currentTime) //有的话直接跳到之前播放位置开始播放
    }
    this.videoContext.play(); //否则直接开始播放
  },
  //监听视频播放进度的回调
  handleTimeUpdate(event) {
    ...
    //判断播放的视频是否已经存在，存在则更新时间，不存在则push到记录播放时长列表中
    let {
      videoUpdateTime
    } = this.data;
  ...
  onShareAppMessage({from}) {
    ...
    if(from === 'button'){
      ...
      return {
        title: '来自menu的转发',
        path: '/pages/video/index',
        imageUrl: '/static/images/recommendSong/08.jpg'
      }
    }
  }
})